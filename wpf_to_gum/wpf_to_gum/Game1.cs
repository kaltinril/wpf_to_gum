...
namespace wpf_to_gum
{
    public class Game1 : Game
    {
        ...
        {
            var buttonBackground = textbox.GetVisual<ColoredRectangleRuntime>();
            var text = textbox.GetVisual<TextRuntime>();

            var enabledState = textbox.GetState(FrameworkElement.EnabledStateName);
            enabledState.Clear();
            enabledState.Apply = () =>
            {
            ...
            var highlightedState = textbox.GetState(FrameworkElement.HighlightedStateName);
            ...
            var focusedState = textbox.GetState(FrameworkElement.FocusedStateName);
            focusedState.Clear();
            focusedState.Apply = () =>
            {
                buttonBackground.Color = Color.Tan;
                text.Color = Color.Black;
            };
        ...
        private void ApplyStateChangesToButton(Button button, Color fontColor, Color enabledBackColor)
        {
            ...
            var text = button.GetVisual<TextRuntime>();

            var enabledState = button.GetState(FrameworkElement.EnabledStateName);
            enabledState.Clear();
            enabledState.Apply = () =>
            {
                buttonBackground.Color = enabledBackColor;
            ...
            var pushedState = button.GetState(FrameworkElement.PushedState);
            pushedState.Clear();
            pushedState.Apply = () =>
            {
                buttonBackground.Color = Color.SlateGray;
                text.Color = fontColor;
            };
